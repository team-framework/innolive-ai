        ...
        try:
            while True:
                ...
                while next_sequence in pending:
                    ...
                    response.processing_ms += (time.perf_counter() - ready_at) * 1_000
                    yield response
                    next_sequence += 1
            await producer
        finally:
...
class PassthroughServicer(ai_processor_pb2_grpc.AiProcessorServicer):
    """Transport-only implementation used by the existing gRPC benchmarks."""

    async def ProcessVideo(self, request_iterator, context) -> AsyncIterator:
        async for request in request_iterator:
            yield messages.ProcessedVideoChunk(
                data=request.data,
                status_message="success",
                timestamp=request.timestamp,
                frame_id=request.frame_id,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="InnoLive gRPC head anonymizer")
    ...
    parser.add_argument(
        "--processing-mode",
        choices=("anonymize", "passthrough"),
        default=os.getenv("GRPC_PROCESSING_MODE", "anonymize"),
        help="passthrough isolates gRPC transport performance",
    )
...
async def serve(
    ...
    try:
        ...
        if isinstance(servicer, AiProcessorServicer):
