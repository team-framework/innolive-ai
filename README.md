...
track metadata를 반환합니다. 운영 진입점은 bidirectional streaming gRPC
...
기본 bind는 `127.0.0.1:50051`, 기본 동시 stream 수는 1입니다.
...
async def main() -> None:
    ...
    async with VideoProcessorClient("127.0.0.1:50051") as client:
        async for result in client.process_jpegs(frames):
...
`client.process_video()`에 전달합니다. client는 JPEG 크기/형식, frame ID 단조 증가,
...
- `faces`: bbox, polygon, confidence, track ID, hold 상태
