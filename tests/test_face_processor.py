...
from service.face_processor import (
    BATCH_SIZE,
    BoTSortTracker,
    Face,
    FaceProcessorPool,
...
class FaceProcessorTest(unittest.TestCase):
    ...
    def test_bot_sort_holds_head_mask_across_short_detection_gap(self):
        ...
        tracker = BoTSortTracker(mask_hold_frames=2, unconfirmed_hold_frames=1)

        detected = tracker.update([head], image)
        first_gap = tracker.update([], image)
        second_gap = tracker.update([], image)
        expired = tracker.update([], image)

        self.assertEqual(detected[0].track_id, 1)
        self.assertEqual(first_gap[0].track_id, 1)
        self.assertEqual(second_gap[0].track_id, 1)
