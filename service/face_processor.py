...
class Settings:
    ...
    def from_env(cls) -> Settings:
        ...
        engine = PROJECT_ROOT / "models" / "best_b4.engine"
        checkpoint = PROJECT_ROOT / "models" / "best.pt"
        model_path = Path(configured_model) if configured_model else checkpoint
        if (
            not configured_model
            and engine.exists()
            and engine.stat().st_mtime >= checkpoint.stat().st_mtime
        ):
            model_path = engine
        ...
        default_single_model = PROJECT_ROOT / "models" / "best_b1.engine"
        single_model_path = (
            Path(configured_single_model)
            if configured_single_model
            else default_single_model
            if default_single_model.exists()
            and default_single_model.stat().st_mtime >= checkpoint.stat().st_mtime
            else None
        )
        ...
        return cls(
            ...
            confidence=float(os.getenv("AI_CONFIDENCE", "0.10")),
            image_size=int(os.getenv("AI_IMAGE_SIZE", "640")),
            mask_hold_frames=int(os.getenv("AI_MASK_HOLD_FRAMES", "8")),
            unconfirmed_hold_frames=int(os.getenv("AI_UNCONFIRMED_HOLD_FRAMES", "2")),
            decode_workers=int(
...
class YoloFaceSegmenter:
    def __init__(
        ...
        self._half = (
            normalized_device.isdigit() or normalized_device.startswith("cuda")
        ) and model_path.suffix != ".engine"
        self._lock = threading.Lock()
        self._warm_up()

    def _validate_model(self, model_path: Path) -> None:
        names = {int(key): str(value) for key, value in self._model.names.items()}
        if self._model.task != "segment" or names.get(0, "").lower() != "head":
            raise ValueError(
                "expected a segment model with class mapping 0: head, "
                f"got task={self._model.task!r}, names={names!r} from {model_path}"
            )
...
class BoTSortTracker:
    def __init__(self, mask_hold_frames: int = 8, unconfirmed_hold_frames: int = 2):
        ...
        self._tracked_faces: dict[int, _TrackedFace] = {}
        self._unconfirmed_faces: list[_TrackedFace] = []
        self._mask_hold_frames = max(0, mask_hold_frames)
        self._unconfirmed_hold_frames = max(0, unconfirmed_hold_frames)
    ...
    def update(self, faces: Sequence[Face], image: np.ndarray) -> tuple[Face, ...]:
        ...
        for track in tracks:
            ...
            if not 0 <= detection_index < len(faces):
                continue
            local_id = self._local_ids.setdefault(
                raw_track_id, len(self._local_ids) + 1
            )
            tracked_face = replace(faces[detection_index], track_id=local_id)
            output[detection_index] = tracked_face
            self._tracked_faces[raw_track_id] = _TrackedFace(tracked_face)
            active_raw_ids.add(raw_track_id)
            active_bboxes.append(tracked_face.bbox)

        self._hold_unconfirmed_faces(output, active_bboxes)

        lost_tracks = {
            int(track.track_id): tuple(float(value) for value in track.xyxy)
            for track in self._tracker.lost_stracks
        }
        for raw_track_id in list(self._tracked_faces):
            if raw_track_id in active_raw_ids:
                continue
            cached = self._tracked_faces[raw_track_id]
            predicted_bbox = lost_tracks.get(raw_track_id)
            if predicted_bbox is None or cached.missed_frames >= self._mask_hold_frames:
                del self._tracked_faces[raw_track_id]
                continue
            cached.face = self._warp_face(cached.face, predicted_bbox)
            cached.missed_frames += 1
            output.append(cached.face)

        return tuple(output)

    def _hold_unconfirmed_faces(
        self,
        output: list[Face],
        active_bboxes: Sequence[tuple[float, float, float, float]],
    ) -> None:
        current = [
            _TrackedFace(face)
            for face in output
            if face.track_id is None and face.confidence >= 0.25
        ]
        next_pending: list[_TrackedFace] = []
        matched_current: set[int] = set()

        for cached in self._unconfirmed_faces:
            if any(
                self._bbox_iou(cached.face.bbox, bbox) >= 0.30 for bbox in active_bboxes
            ):
                continue
            best_index = -1
            best_iou = 0.0
            for index, candidate in enumerate(current):
                if index in matched_current:
                    continue
                iou = self._bbox_iou(cached.face.bbox, candidate.face.bbox)
                if iou > best_iou:
                    best_index, best_iou = index, iou
            if best_index >= 0 and best_iou >= 0.30:
                next_pending.append(current[best_index])
                matched_current.add(best_index)
            elif cached.missed_frames < self._unconfirmed_hold_frames:
                cached.missed_frames += 1
        ...
        self._unconfirmed_faces = next_pending
    ...
    def _warp_face(
        ...
        old_bbox = face.bbox
        old_width = max(old_bbox[2] - old_bbox[0], 1.0)
        old_height = max(old_bbox[3] - old_bbox[1], 1.0)
        scale_x = max(new_bbox[2] - new_bbox[0], 1.0) / old_width
        scale_y = max(new_bbox[3] - new_bbox[1], 1.0) / old_height

        def warp(points: Sequence[tuple[float, float]]):
            return tuple(
                (
                    round(new_bbox[0] + (x - old_bbox[0]) * scale_x, 1),
                    round(new_bbox[1] + (y - old_bbox[1]) * scale_y, 1),
                )
                for x, y in points
            )
        ...
        return replace(
            face,
            bbox=tuple(round(value, 1) for value in new_bbox),
            polygon=warp(face.polygon),
            metadata_polygon=warp(face.transport_polygon),
        )
...
class _InferenceBatchWorker:
    ...
    def _track(
        ...
        group: Sequence[tuple[_BatchItem, np.ndarray, list[Face], ProcessingTiming]],
        submitted_at: float,
    ...
    def __init__(
        ...
        self._tracker_factory = tracker_factory or (
            lambda: BoTSortTracker(
                settings.mask_hold_frames,
                settings.unconfirmed_hold_frames,
            )
        )
